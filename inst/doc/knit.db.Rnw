...
%\texttt{PEP.db} includes a constructor that will generate the appropriate object from a \texttt{data.frame} that has at least a column for each requirement and any additional information.
%<<constructor-df>>=
%@
%
...
%The objects being \texttt{RangedData}, it is naturally possible to use an \texttt{IRanges} object to specify the ranges of the peptides.
<<constructor-rd>>=
...
The second and last step to generate a new PEP.db collection is to compute the Zscores for the given peptides.
<<add-zScores>>=
pepZscore(newDB) <- computeZscore
